...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "zcode") {
    return workspacePath ? join(baseDir, ".zcode-local") : join(baseDir, ".zcode-local", "cli");
  }
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".zcode-local", "config.json");
