...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".zcode-local", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".zcode-local", "cli");
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.zcode-local/skills/<name>` 下的目录项本身。
