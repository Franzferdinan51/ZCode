...
export const skillToolEntry: ToolEntry = {
  ...
  metadata: {
    name: "Skill",
    description: `Invoke a named skill (specialized capability + domain instructions) in this conversation.

- Set \`skill\` to the exact name the user typed as \`/name\` (no leading slash), or one listed in a system reminder. For plugin skills use the fully qualified \`plugin:skill\` form; pass optional \`args\`.
- Never invent a skill name — an unknown skill fails. Do not use this for built-in CLI commands (/help, /clear, …).
- When a skill matches the request, call it BEFORE any other response about the task — never just mention it.
- Do not call a skill that is already running.
- If a <command-name> tag already appears in this turn, the skill is loaded — follow it directly instead of calling this tool again
`,
    readOnly: true,
    destructive: false,
    concurrentSafe: true,
