...
Plugin state lives under `~/.zcode-local/cli/plugins`:
...
zcode reads hooks from the same main JSON config file as MCP, usually `~/.zcode-local/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
